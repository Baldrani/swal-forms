  ...
  extend(SwalForm.prototype, {
    ...
    getFormValues: function (isConfirm) {
      ...
      function toValuableAttrs (tag) {
        //Maël : A Améliorer (Fusionner les 2 parties)
        var attr = {};
        attr[tag.id || tag.name] = tag.value;
        var p = document.createElement("p");
        var warnMsg = 'Ce champ est obligatoire';
        if (isConfirm && tag.dataset.swalFormsRequired && !tag.value) {
          tag.style.borderColor = "#8e3535";
          tag.style.boxShadow = "0 0 5px 1px #862020";
          p.style.color = "#b30000";
          warnTextNode && warnTextNode.remove && warnTextNode.remove();
          warnTextNode = document.createTextNode(warnMsg);
          p.appendChild(warnTextNode);
          tag.parentNode.insertBefore(p, tag.nextSibling);
          throw new Error(warnMsg);
        }
        if(isConfirm && tag.dataset.swalFormsRequired &&  !tag.value.match(tag.pattern)){
          tag.style.borderColor = "#8e3535";
          tag.style.boxShadow = "0 0 5px 1px #862020";
          p.style.color = "#b30000";
          warnMsg = "Le champ n'est pas correctement rempli";
          warnTextNode && warnTextNode.remove && warnTextNode.remove();
          warnTextNode = document.createTextNode(warnMsg);
          p.appendChild(warnTextNode);
          tag.parentNode.insertBefore(p, tag.nextSibling);
